...
@Service
public class QiniuUploadService00 {
    ...
    public String uploadFile(byte[] fileBytes, String fileName, String directory) throws QiniuException {
        ...
        // 生成唯一的文件名
        String fullPath = directory + "/" + randomFileName+ fileName;
    ...
    public void deleteFile(String avatarUrl) throws QiniuException {
        // 提取文件路径，从域名之后开始
        String filePath = avatarUrl.substring(avatarUrl.indexOf("/", avatarUrl.indexOf("//") + 2) + 1);


        try {
            Response response = bucketManager.delete(BUCKET_NAME, filePath);
            if (response.isOK()) {
                System.out.println("删除成功，删除文件路径: " + filePath);
            } else {
                throw new QiniuException(response);
            }
